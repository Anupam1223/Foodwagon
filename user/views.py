...
from .forms import UserUpdateForm, UserUpdateeForm
...
'''def updateUser(request):
    if request.method == "POST":
        ...
        if not profile_pic:
            data = {
                'error': 'please provide profile pic'
            }
            return JsonResponse(data)
        ...
        User.objects.filter(id=userid).update(first_name = name)
        User.objects.filter(id=userid).update(email = email)
        User.objects.filter(id=userid).update(profile_pic = profile_pic)
        data = {
            'success': 'password changed successfully'
        }
        return JsonResponse(data)'''
