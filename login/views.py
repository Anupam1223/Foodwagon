...
def loginUser(request):
    ...
    if request.method == "POST":
        ...
        if logins:
            ...
            if not valemail:
                data = {
                    'error': 'please enter your your email'
                }
            ...
            if not verifyUser.is_active:
                data = {
                    'error': 'user is not active, check your mail'
                }
                return JsonResponse(data)
            ...
            if not check_password(valpassword, verifyUser.password):
                data = {
                    'error': 'username and  password didnt matched'
                }
                return JsonResponse(data)
            ...
            data = {
                'error': 'data cannot be sent, try again'
            }
            return JsonResponse(data)
...
def UserRegister(request):
    ...
    if request.method == "POST":
        ...
        if logins:
                ...
                email.send()
                data = {
                    'success': 'check mail to verify account'
                }
                return JsonResponse(data)
        else:
            data = {
